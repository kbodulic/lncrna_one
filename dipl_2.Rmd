...
ggsave(lengths_plot,filename = "lengths_plot.jpg",width = 7,height = 5)
...
isoforms_plot_proportions<-ggplot(data=isoforms_table_together_num,aes(x=number_of_isoforms,y=proportion,fill=category)) +
  ...
  scale_fill_manual(values=c("indianred2","steelblue"), name = "Kategorija", labels=c("lncRNA-kodirajući geni","protein-kodirajući geni")) +
...
write.table(protein_coding_annotation_save[feature=="mRNA",.SD,.SDcols="attribute"],file="esu_met_prot_ex.txt",quote = F,col.names = F,row.names = F,sep = "\t")
...
lncrna_gtf_mrna<-data.table(seqname=filtered_together_paf_union$target_name,source="AUGUSTUS",feature="mRNA",start=filtered_together_paf_union$target_start,end=filtered_together_paf_union$target_end,score=0.5,strand_ignore="+",frame=".",attribute=filtered_together_paf_union$query_name)
...
element_length_boxplot<-ggplot(data=element_width_table,aes(x=category,y=width,fill=category)) +
  ...
  scale_y_log10() +
  theme_bw() +
  scale_fill_manual(values=c("indianred2","steelblue"),name="Kategorija",label=c("lncRNA-kodirajući geni","protein-kodirajući geni")) +
  theme(legend.position = "top") +
  theme(legend.title = element_text(size=9,face="bold")) +
...
splice_sites_table_total_together[,category:=ifelse(category=="lncRNA","lncRNA geni","protein-kodirajući geni")]
...
protein_transcripts_clean_granges_split<-protein_transcripts_clean_granges_split[names(protein_transcripts_clean_granges_split)%in%names(protein_exons_clean_split)]
...
number_of_lncrna_classes_prikaz
...
class_lengths_plot<-ggplot(data=class_together_table,aes(x=class,y=element_length,fill=class)) +
  ...
  scale_fill_hp(discrete = TRUE, option = "NewtScamander", name = "Klasa",labels=c("Intronske lncRNA","Intergenske lncRNA","Preklapajuće lncRNA")) +
  ylab("Duljina / pb (log10 skalirano)") 
...
inter_prot_cod_distance_plot<-ggplot(data=intergenic_prot_nearest_table,aes(x=distance)) +
  ...
  theme_bw()  +
  xlab("Udaljenost lncRNA-kodirajućeg gena od najbližeg protein-kodirajućeg gena / pb") +
  ylab("Broj lncRNA-kodirajućih gena") +
...
gc_density_plot<-ggplot(data=gc_table,aes(x=transcript_gc,y=..scaled..,fill=category)) +
  ...
  xlab("udio GC tranksripata") +
  ylab("Udio transkripta") +
  theme(legend.title = element_text(size=9,face="bold"))
...
exons_clean_lncrna_rnaspades<-rnaspades_filtered_exons[transcript%in%rnaspades_filtered_list]
exons_clean_lncrna_rnaspades[,gene:=str_extract(transcript,"NODE_\\d*")]
exons_clean_lncrna_trinity<-trinity_filtered_exons[transcript%in%trinity_filtered_list]
...
overlaps_repeat_together_num_table_20[class=="overlapping_lncRNA",class:="preklapajuće lncRNA"]
overlaps_repeat_together_num_table_20[class=="protein_coding_genes",class:="protein kodirajući geni"]
...
overlaps_repeat_together_num_table_normalized_sum_bases_20[class=="overlapping_lncRNA",class:="Preklapajuće lncRNA"]
overlaps_repeat_together_num_table_normalized_sum_bases_20[class=="protein_coding_genes",class:="Protein-kodirajući geni"]
...
overlaps_repeat_together_num_table_normalized_sum_bases_20_2[class=="overlapping_lncRNA",class:="Preklapajuće lncRNA"]
overlaps_repeat_together_num_table_normalized_sum_bases_20_2[class=="protein_coding_genes",class:="Protein-kodirajući geni"]
...
numb_gene_in_genomes_plot<-ggplot(data=numb_gene_in_genomes_table,aes(x=number_of_genomes,y=number_of_lncrna)) +
  ...
  theme_bw() +
  xlab("Broj vrsta s BLASTn pogotcima pojedine lncRNA") +
  ylab("Broj lncRNA")
...
em_50_filtered<-blast_gr_q_split_reduce_unlist_dt_lengths[species=="Ephydatia_mulleri_transcriptome" & sumqperclen>=100]
em_50_filtered[,sseqid:=str_extract(sseqid,"(?<=transcriptome_).*")]
em_50_fortrin<-em_50_filtered$sseqid
emu_trans_length.txt<-fread("emu_trans_length.txt")
setnames(emu_trans_length.txt,c("transcript_id","length"))
trinotate_em<-fread("trinotate_annotation_report_Emu_tri.xls")
trinotate_em<-trinotate_em[,.SD,.SDcols=c("transcript_id","prot_id","prot_coords")]
trinotate_em[,':='(prot_start=as.numeric(str_extract(prot_coords,"[[:digit:]]+(?=\\-)")),prot_end=as.numeric(str_extract(prot_coords,"(?<=\\-)[[:digit:]]+")))]
trinotate_em[,':='(orf_length=abs(prot_end-prot_start),prot_length=round(abs(prot_end-prot_start)/3,0))]
trinotate_em<-merge(trinotate_em,emu_trans_length.txt)
em_50_filtered<-em_50_filtered[,.SD,.SDcols=c("qseqid","sseqid")]
setnames(em_50_filtered,c("qseqid","transcript_id"))
trinotate_em_merged<-merge(trinotate_em,em_50_filtered,by="transcript_id")
trinotate_em_merged[is.na(orf_length),orf_length:=0]
trinotate_em_merged[,is_min_orf:=ifelse(.SD==min(.SD),T,F),.SDcols="orf_length",by=qseqid]
trinotate_em_merged<-unique(trinotate_em_merged[is_min_orf==T],by="qseqid")
trinotate_em_merged[length > 200 & orf_length<=150,.N]
...
read_counts_table<-fread("day1_day10_rna_to_genome_dup_counts.txt")
read_counts_table<-read_counts_table[,.SD,.SDcols=c("Geneid","Length","../sorted_RNAseq_day1_to_Esu_genome.bam","../sorted_RNAseq_day10_to_Esu_genome.bam")]
...
read_counts_table_longer<-as.data.table(pivot_longer(data=read_counts_table,cols = c("FPKM_day1","FPKM_day10"),names_to = "experiment",values_to = "FPKM",names_prefix = "FPKM_"))
fpkm_prot_lncrna_plot<-ggplot(data=read_counts_table_longer,aes(x=category,y=FPKM,fill=category)) +
  geom_boxplot() +
  theme_bw() +
  ...
  theme(legend.position = "none") +
  theme(axis.title.x=element_blank())
fpkm_prot_lncrna_plot
ggsave(fpkm_prot_lncrna_plot,file="fpkm_prot_lncrna_plot.jpg")
...
read_counts_table_lncrna_class[class=="overlapping_lncRNA",class:="Preklapajuće lncRNA"]
...
ggsave(fpkm_lncrna_class_plot,file="fpkm_lncrna_class_plot.jpg",width = 6,height = 5)
...
#samples for randfold

exonic_lncrnas_randfold<-unique(lncrna_repeat_overlap_category_class_merged_copy$gene)[sample(1:length(unique(lncrna_repeat_overlap_category_class_merged_copy$gene)),size=100,replace = F)]

other_lncrnas_randfold<-unique(lncrna_no_repeats_dt_merged$gene)[sample(1:length(unique(lncrna_no_repeats_dt_merged$gene)),size = 100,replace = F)]


write.table(exonic_lncrnas_randfold,file="exonic_lncrnas_randfold.txt",row.names = F,col.names = F,quote = F,sep = "\t")
write.table(other_lncrnas_randfold,file="other_lncrnas_randfold.txt",row.names = F,col.names = F,quote = F,sep = "\t")
...
randfold_res<-fread("randfold_res50.txt",header = F)
randfold_res_revcomp<-fread("randfold_res50_revcomp.txt",header = F)
...
100*randfold_together_one[p_value<=0.05,.N]/randfold_together_one[,.N]
100*randfold_together_one[p_value<=0.1,.N]/randfold_together_one[,.N]
pval_dist_plot<-ggplot(randfold_together_one,aes(x=p_value)) +
  geom_histogram(fill="indianred2",col="black",binwidth = 0.05) +
  theme_bw() +
  xlab("p-vrijednost") +
  ylab("Broj molekula lncRNA")
pval_dist_plot
ggsave(pval_dist_plot,file="pval_dist_plot.jpg",width=6,height = 5)
...
class_pval_plot<-ggplot(randfold_together_one_merged,aes(x=class,y=p_value,fill=class)) +
  geom_boxplot() +
  theme_bw() +
  scale_fill_hp(discrete = TRUE, option = "NewtScamander", name = "Klasa",labels=c("Intergenske lncRNA","Intronske lncRNA","Preklapajuće lncRNA")) +
  theme(legend.position = "top") +
  theme(axis.title.x = element_blank()) +
  theme(axis.text.x = element_blank())  +
  ylab("p-vrijednost") +
...
intronic_prots_dt<-unique(overlaps_lncRNA_prot_within_table[query_name%in%intersect(overlapping_lncrna,intronic_lncrna)==F])
intronic_prots_dt<-merge(intronic_prots_dt,isoform_map_table_together_cp2,by="query_name")
intronic_prots_dt<-unique(intronic_prots_dt,by=c("attribute","gene_id"))
intronic_prots_dt[,distance:=0]

#overlapping

overlapping_prots_dt<-unique(overlaps_prot_lncrna_within_table)
overlapping_prots_dt<-merge(overlapping_prots_dt,isoform_map_table_together_cp2,by="query_name")
overlapping_prots_dt<-unique(overlapping_prots_dt,by=c("attribute","gene_id"))
...
rnaseq_paf<-fread("rnaseq_together_to_genome_test.paf")[1:100,1:12]
setnames(rnaseq_paf,c("query_name","query_length","query_start","query_end","strand_ignore","target_name","target_length","target_start","target_end","matches","alignment_block","mapping_quality"))

rnaseq_gr<-makeGRangesFromDataFrame(rnaseq_paf,seqnames.field = "target_name",start.field = "target_start", end.field = "target_end",ignore.strand = T )

lncrna_rna_overlaps<-findOverlaps(lncrnas_final_reduced_together,rnaseq_gr,ignore.strand=T)
lncrna_rna_overlaps_dt<-cbind(as.data.table(lncrnas_final_reduced_together[queryHits(lncrna_rna_overlaps)]),as.data.table(rnaseq_gr[subjectHits(lncrna_rna_overlaps)]))
setnames(lncrna_rna_overlaps_dt,c("seqnames1","start1","end1","width1","strand1","lncrna_g","gene_id","seqnames2","start2","end2","width2","strand2"))

exons_gr<-makeGRangesFromDataFrame(table_adj_lncrna_prot_together_merge[,.SD,.SDcols=c("attribute","gene_id","seqnames_exon","start_exon","end_exon")],seqnames.field = "seqnames_exon",start.field="start_exon",end.field = "end_exon",ignore.strand = T,keep.extra.columns = T)
exons_rna_overlaps<-findOverlaps(exons_gr,rnaseq_gr,ignore.strand=T)
exons_rna_overlaps_dt<-cbind(as.data.table(exons_gr[queryHits(exons_rna_overlaps)]),as.data.table(rnaseq_gr[subjectHits(exons_rna_overlaps)]))
setnames(exons_rna_overlaps_dt,c("seqnames1","start1","end1","width1","strand1","attribute","gene_id","seqnames2","start2","end2","width2","strand2"))

unique(merge(lncrna_rna_overlaps_dt,exons_rna_overlaps_dt,by=c("seqnames2","start2","end2")),by=c("gene_id.x","attribute"))

unique(merge(lncrna_rna_overlaps_dt,exons_rna_overlaps_dt,by=c("seqnames2","start2","end2")),by=c("gene_id.x"))
rm(rnaseq_paf)

write.table(lncrnas_final_reduced_together_dt,file="lncrnas_final_reduced_together_dt.txt",quote = F,col.names = T,row.names = F)
```
...
read_counts_table_lncrna<-read_counts_table[category=="lncRNA"]
read_counts_table_lncrna<-read_counts_table_lncrna[counts!=0]
read_counts_table_lncrna[,gene_id:=as.double(gene_id)]
read_counts_table_lncrna_class<-merge(read_counts_table_lncrna,lncRNA_classification_table_with_gene_id,by="gene_id")
...
kruskal.test(x = c(read_counts_table_lncrna_class[class=="Intergenske lncRNA"]$FPKM,read_counts_table_lncrna_class[class=="Intronske lncRNA"]$FPKM,read_counts_table_lncrna_class[class=="Preklapajuce lncRNA"]$FPKM),g=c(rep(1,length(read_counts_table_lncrna_class[class=="Intergenske lncRNA"]$FPKM)),rep(2,length(read_counts_table_lncrna_class[class=="Intronske lncRNA"]$FPKM)),rep(3,length(read_counts_table_lncrna_class[class=="Preklapajuce lncRNA"]$FPKM))))
...
anova_fpkm<-aov(FPKM~cate,data = lncrna_repeat_overlap_category_class_merged_copy_merged_with_no_rep_cp)
TukeyHSD(anova_fpkm)
#overlapping proteisn vs non-overlapping proteins
...
protein_coding_annotation<-protein_coding_annotation[,1:10]
protein_coding_annotation[feature=="gene",numb_prot_scaff:=nrow(.SD),by=seqname]
one_prot_scaff<-protein_coding_annotation[numb_prot_scaff==1,seqname]

protein_coding_annotation_more_scaff<-protein_coding_annotation[seqname%in%one_prot_scaff==F]

prot_gr<-makeGRangesFromDataFrame(df = protein_coding_annotation_more_scaff[feature=="gene"],ignore.strand = T,keep.extra.columns = T)

prot_prot_nearest_dt<-data.table(first_prot_pair_seqname=as.character(seqnames(prot_gr)),first_prot_pair_start=as.double(start(prot_gr)),first_prot_pair_end=as.double(end(prot_gr)),first_prot_pair_attribute=prot_gr$attribute,second_prot_pair_seqname=as.character(seqnames(prot_gr[nearest(prot_gr)])),second_prot_pair_start=as.double(start(prot_gr[nearest(prot_gr)])),second_prot_pair_end=as.double(end(prot_gr[nearest(prot_gr)])),second_prot_pair_attribute=prot_gr[nearest(prot_gr)]$attribute)

prot_prot_nearest_dt[,distance:=ifelse(first_prot_pair_start>second_prot_pair_start,first_prot_pair_start-second_prot_pair_end-1,first_prot_pair_end-second_prot_pair_start+1)]
prot_gr$attribute_second<-prot_gr[nearest(prot_gr)]$attribute
first_prot_rna_ovl<-findOverlaps(rnaseq_gr,prot_gr)
first_prot_rna_ovl_dt<-cbind(as.data.table(rnaseq_gr[queryHits(first_prot_rna_ovl)]),as.data.table(prot_gr[subjectHits(first_prot_rna_ovl)]))
setnames(first_prot_rna_ovl_dt,c("seqnames_rna","start_rna","end_rna","width_rna","strand_rna","seqnames_first","start_first","end_first","width_first","strand_first","source","feature","score","strand_ignore","frame","attribute_first","numb_prot_scaff","attribute_second"))

prot_gr$attribute_second<-NULL
second_prot_rna_ovl<-findOverlaps(rnaseq_gr,prot_gr[nearest(prot_gr)])
second_prot_rna_ovl_dt<-cbind(as.data.table(rnaseq_gr[queryHits(second_prot_rna_ovl)]),as.data.table(prot_gr[nearest(prot_gr)][subjectHits(second_prot_rna_ovl)]))
setnames(second_prot_rna_ovl_dt,c("seqnames_rna","start_rna","end_rna","width_rna","strand_rna","seqnames_second","start_second","end_second","width_second","strand_second","source","feature","score","strand_ignore","frame","attribute_second",""))


prot_prot_pairs_dt<-merge(first_prot_rna_ovl_dt,second_prot_rna_ovl_dt,by=c("seqnames_rna","start_rna","end_rna","attribute_second"))
...
only_max_sumperceln_width_filtered_all_width<-only_max_sumperceln_width_filtered_all_width[species=="Ephydatia_mulleri_transcriptome"]

em_transcripts_table<-data.table(transcript=only_max_sumperceln_width_filtered_all_width$sseqid)
em_transcripts_table[,transcript:=str_remove(transcript,"Ephydatia_mulleri_transcriptome_")]
...
proteins_final_reduced_together_20kb<-protein_coding_genes_granges[start(protein_coding_genes_granges)>25000]
proteins_final_reduced_together_20kb_short<-protein_coding_genes_granges[start(protein_coding_genes_granges)<=25000]
end(proteins_final_reduced_together_20kb_short)<-start(proteins_final_reduced_together_20kb_short)-1
start(proteins_final_reduced_together_20kb_short)<-(rep(1,length(proteins_final_reduced_together_20kb_short)))
new_starts<-start(proteins_final_reduced_together_20kb)-25000
start(proteins_final_reduced_together_20kb)<-new_starts
proteins_final_reduced_together_20kb_final<-c(proteins_final_reduced_together_20kb,proteins_final_reduced_together_20kb_short)
proteins_final_reduced_together_20kb_final<-proteins_final_reduced_together_20kb_final[proteins_final_reduced_together_20kb_final$attribute!="jg27231"]
...
protein_coding_genes_merge<-plyr::join(protein_coding_genes,contig_length_table,by="seqname")


proteins_final_reduced_together_20kb_right<-protein_coding_genes_granges[end(protein_coding_genes_granges)<(protein_coding_genes_merge$cont_length-25000)]
proteins_final_reduced_together_20kb_short_right<-protein_coding_genes_granges[end(protein_coding_genes_granges)>(protein_coding_genes_merge$cont_length-25000)]

proteins_final_reduced_together_20kb_short_right_dt<-as.data.table(proteins_final_reduced_together_20kb_short_right)
colnames(proteins_final_reduced_together_20kb_short_right_dt)[1]<-"seqname"
proteins_final_reduced_together_20kb_short_right_dt_merge<-plyr::join(proteins_final_reduced_together_20kb_short_right_dt,contig_length_table,by="seqname")

end(proteins_final_reduced_together_20kb_short_right)<-proteins_final_reduced_together_20kb_short_right_dt_merge$cont_length

end(proteins_final_reduced_together_20kb_right)<-end(proteins_final_reduced_together_20kb_right)+25000


proteins_final_reduced_together_20kb_final_dt<-as.data.table(proteins_final_reduced_together_20kb_final)

proteins_final_reduced_together_20kb_short_right_dt<-as.data.table(proteins_final_reduced_together_20kb_short_right)
colnames(proteins_final_reduced_together_20kb_short_right_dt)[3]<-"end_short"

proteins_final_reduced_together_20kb_right_dt<-as.data.table(proteins_final_reduced_together_20kb_right)
colnames(proteins_final_reduced_together_20kb_right_dt)[3]<-"end_norm"

proteins_final_reduced_together_20kb_dt_merged<-merge(proteins_final_reduced_together_20kb_final_dt,proteins_final_reduced_together_20kb_short_right_dt[,c(3,11)],by="attribute",all.x=T)

proteins_final_reduced_together_20kb_dt_merged2<-merge(proteins_final_reduced_together_20kb_dt_merged,proteins_final_reduced_together_20kb_right_dt[,c(3,11)],by="attribute",all.x=T)

proteins_final_reduced_together_20kb_dt_merged2[is.na(end_short),end:=end_norm]
proteins_final_reduced_together_20kb_dt_merged2[is.na(end_norm),end:=end_short]

proteins_final_reduced_together_20kb_dt_merged2_gr<-makeGRangesFromDataFrame(proteins_final_reduced_together_20kb_dt_merged2,keep.extra.columns = T)




prot_overlaps_20kb_2<-findOverlaps(lncrnas_final_reduced_together,proteins_final_reduced_together_20kb_dt_merged2_gr)
prot_overlaps_20kb_2_dt<-cbind(as.data.table(lncrnas_final_reduced_together[queryHits(prot_overlaps_20kb_2)]),as.data.table(proteins_final_reduced_together_20kb_dt_merged2_gr[subjectHits(prot_overlaps_20kb_2)]))
setnames(prot_overlaps_20kb_2_dt,c("seqnames1","start1","end1","width1","strand1","gene","gene_id","seqnames2","start2","end2","width2","strand2","attribute","source","feature","score","strand_ignore","frame","end_short","end_norm"))
```
...
go_all_esu_nonempty<-go_all_esu [grepl("C:",go_names)==F]
...
prot_overlaps_20kb_dt_GO_longer<-prot_overlaps_20kb_dt_GO_longer[grepl("^ ",GO_term)==F]
...
prot_overlaps_20kb_dt_GO_rev<-merge(prot_overlaps_20kb_2_dt,go_all_esu_nonempty[,c(3,11)],by="attribute")

max_split_rev<-max(str_count(prot_overlaps_20kb_dt_GO_rev$go_names,";"))+1
prot_overlaps_20kb_dt_GO_rev[,paste("GO_split",1:max_split,sep="_"):=tstrsplit(go_names, ";")]

prot_overlaps_20kb_dt_GO_longer_rev<-as.data.table(pivot_longer(prot_overlaps_20kb_dt_GO_rev,cols = grep("GO_split",colnames(prot_overlaps_20kb_dt_GO),value = T),values_to = "GO_term"))
prot_overlaps_20kb_dt_GO_longer_rev<-prot_overlaps_20kb_dt_GO_longer_rev[grepl("^ ",GO_term)==F]
prot_overlaps_20kb_dt_GO_longer_rev<-prot_overlaps_20kb_dt_GO_longer_rev[is.na(GO_term)==F]


prot_overlaps_20kb_dt_GO_longer_rev[grepl("P:",GO_term),GO_category:="P"]
prot_overlaps_20kb_dt_GO_longer_rev[grepl("F:",GO_term),GO_category:="F"]



prot_overlaps_20kb_dt_GO_longer_rev_f<-prot_overlaps_20kb_dt_GO_longer_rev[GO_category=="F"]
prot_overlaps_20kb_dt_GO_longer_rev_f[,GO_term:=str_remove(GO_term,"F:")]
prot_overlaps_20kb_dt_GO_longer_rev_f_N<-prot_overlaps_20kb_dt_GO_longer_rev_f[,.N,by=c("gene_id","GO_term")][order(-N)]
prot_overlaps_20kb_dt_GO_longer_rev_f_N_all<-prot_overlaps_20kb_dt_GO_longer_rev_f_N[,.N,by=GO_term][order(-N)]


prot_overlaps_20kb_dt_GO_longer_rev_f_N_all[,GO_term:=factor(GO_term,levels=prot_overlaps_20kb_dt_GO_longer_rev_f_N_all[1:10]$GO_term)]

ggplot(prot_overlaps_20kb_dt_GO_longer_rev_f_N_all[1:10],aes(x=GO_term,y=N,fill=GO_term)) +
  geom_bar(stat="identity",color="black",alpha=0.9) +
  theme_bw() +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(angle = 45,hjust = 0.9,size = 9)) +
...
prot_overlaps_20kb_dt_GO_longer_rev_p<-prot_overlaps_20kb_dt_GO_longer_rev[GO_category=="P"]
prot_overlaps_20kb_dt_GO_longer_rev_p[,GO_term:=str_remove(GO_term,"P:")]
prot_overlaps_20kb_dt_GO_longer_rev_p_N<-prot_overlaps_20kb_dt_GO_longer_rev_p[,.N,by=c("gene_id","GO_term")][order(-N)]
prot_overlaps_20kb_dt_GO_longer_rev_p_N_all<-prot_overlaps_20kb_dt_GO_longer_rev_p_N[,.N,by=GO_term][order(-N)]

prot_overlaps_20kb_dt_GO_longer_rev_p_N_all[,GO_term:=factor(GO_term,levels=prot_overlaps_20kb_dt_GO_longer_rev_p_N_all[1:10]$GO_term)]

lncrna_go_kb_plot<-ggplot(prot_overlaps_20kb_dt_GO_longer_rev_p_N_all[1:10],aes(x=GO_term,y=N,fill=GO_term)) +
  geom_bar(stat="identity",color="black",alpha=0.9) +
  theme_bw() +
  theme(legend.position = "none") +
  ...
  scale_fill_brewer(palette="Spectral") +
  xlab("GO term") +
  ylab("Number of lncRNAs within 50 kb of a protein")
  
lncrna_go_kb_plot

#trazi

ggsave(lncrna_go_kb_plot,filename = "lncrna_go_kb_plot.png",width=6,height = 5)





#GO distances - whole gene - closest

dist_table_whole<-rbind(intergenic_prot_nearest_table[,.SD,.SDcols=c("gene_id","attribute","distance")],intronic_prots_dt[,.SD,.SDcols=c("gene_id","attribute","distance")],overlapping_prots_dt[,.SD,.SDcols=c("gene_id","attribute","distance")])

dist_table_whole_go_f<-merge(dist_table_whole,prot_overlaps_20kb_dt_GO_longer_f[,.SD,.SDcols=c("attribute","GO_term","GO_category")])

dist_table_whole_go_p<-merge(dist_table_whole,prot_overlaps_20kb_dt_GO_longer_p[,.SD,.SDcols=c("attribute","GO_term","GO_category")])




dist_table_whole_go_f<-dist_table_whole_go_f[GO_term%in%top_go_f]

dist_table_whole_go_p<-dist_table_whole_go_p[GO_term%in%top_go_p]




ggplot(data=dist_table_whole_go_f,aes(x=GO_term,y=distance,fill=GO_term)) +
  geom_boxplot()+
  theme_bw() +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(angle = 45,hjust = 0.9,size = 9))

ggplot(data=dist_table_whole_go_p,aes(x=GO_term,y=distance,fill=GO_term)) +
  geom_boxplot()+
  theme_bw() +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(angle = 45,hjust = 0.9,size = 9))
...
dist_table_whole_go_f<-dist_table_whole_go_f[GO_term%in%top_go_f]
dist_table_whole_go_f[,distance:=abs(distance)]
dist_table_whole_go_f[,log_distance:=log(distance+1)]

dist_table_whole_go_p<-dist_table_whole_go_p[GO_term%in%top_go_p]
dist_table_whole_go_p[,distance:=abs(distance)]
dist_table_whole_go_p[,log_distance:=log(distance+1)]

dist_table_whole_go_f[,GO_term:=factor(GO_term,levels = prot_overlaps_20kb_dt_GO_longer_f_N$GO_term)]
dist_table_whole_go_p[,GO_term:=factor(GO_term,levels = prot_overlaps_20kb_dt_GO_longer_p_N$GO_term)]
...
ggplot(data=dist_table_whole_go_f,aes(x=GO_term,y=log_distance,fill=GO_term)) +
  geom_boxplot()+
  theme_bw() +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(angle = 45,hjust = 0.9,size = 9))

ggplot(data=dist_table_whole_go_p,aes(x=GO_term,y=log_distance,fill=GO_term)) +
  geom_boxplot()+
  theme_bw() +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(angle = 45,hjust = 0.9,size = 9))
...
prot_overlaps_20kb_dt

prot_coding_genes_kb_gr<-makeGRangesFromDataFrame(data.table(seqnames=prot_overlaps_20kb_dt$seqnames2,start=prot_overlaps_20kb_dt$start2,end=prot_overlaps_20kb_dt$end2,attribute=prot_overlaps_20kb_dt$attribute,strand=prot_overlaps_20kb_dt$strand2),keep.extra.columns = T)


prot_overlaps_20kb_dt_lncrna_merge<-merge(prot_overlaps_20kb_dt,lncrnas_final_reduced_together_dt,by="gene_id")

lncrna_genes_kb_gr<-makeGRangesFromDataFrame(data.table(seqnames=prot_overlaps_20kb_dt_lncrna_merge$seqnames1.y,start=prot_overlaps_20kb_dt_lncrna_merge$start1.y,end=prot_overlaps_20kb_dt_lncrna_merge$end1.y,gene_id=prot_overlaps_20kb_dt_lncrna_merge$gene_id,strand=prot_overlaps_20kb_dt_lncrna_merge$strand1.y),keep.extra.columns = T)

prot_overlaps_20kb_dt_lncrna_merge[,distance:=ifelse(start1.y>start1.x,start1.y-end1.x-1,end1.y-start1.x+1)]


dist_table_all_go_f<-merge(prot_overlaps_20kb_dt_lncrna_merge,prot_overlaps_20kb_dt_GO_longer_f[,.SD,.SDcols=c("attribute","GO_term","GO_category")],by="attribute")

dist_table_all_go_p<-merge(prot_overlaps_20kb_dt_lncrna_merge,prot_overlaps_20kb_dt_GO_longer_p[,.SD,.SDcols=c("attribute","GO_term","GO_category")],by="attribute")



dist_table_all_go_f<-dist_table_all_go_f[GO_term%in%top_go_f]

dist_table_all_go_p<-dist_table_all_go_p[GO_term%in%top_go_p]
...
dist_table_all_go_f<-dist_table_whole_go_f[GO_term%in%top_go_f]
dist_table_all_go_f[,distance:=abs(distance)]
dist_table_all_go_f[,log_distance:=log(distance+1)]

dist_table_all_go_p<-dist_table_whole_go_p[GO_term%in%top_go_p]
...
ggplot(data=dist_table_all_go_f,aes(x=GO_term,y=log_distance,fill=GO_term)) +
  geom_boxplot()+
  theme_bw() +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(angle = 45,hjust = 0.9,size = 9))

ggplot(data=dist_table_all_go_p,aes(x=GO_term,y=log_distance,fill=GO_term)) +
  geom_boxplot()+
  theme_bw() +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(angle = 45,hjust = 0.9,size = 9))
...
intergenic_prot_nearest_table_merge_trans<-merge(intergenic_prot_nearest_table,protein_coding_genes[,.SD,.SDcols=c("attribute","trans_start")],by="attribute")
intronic_prots_dt_merge_trans<-merge(intronic_prots_dt,protein_coding_genes[,.SD,.SDcols=c("attribute","trans_start")],by="attribute")
overlapping_prots_dt_merge_trans<-merge(overlapping_prots_dt,protein_coding_genes[,.SD,.SDcols=c("attribute","trans_start")],by="attribute")

dist_table_whole_trans<-rbind(intergenic_prot_nearest_table_merge_trans[,.SD,.SDcols=c("gene_id","attribute","trans_start","start1","end1")],intronic_prots_dt_merge_trans[,.SD,.SDcols=c("gene_id","attribute","trans_start","start1","end1")],intronic_prots_dt_merge_trans[,.SD,.SDcols=c("gene_id","attribute","trans_start","start1","end1")])

dist_table_whole_trans[,start_dis:=abs(start1-trans_start)]
dist_table_whole_trans[,end_dis:=abs(end1-trans_start)]
dist_table_whole_trans[,distance:=ifelse(start_dis<end_dis,start_dis,end_dis)]

dist_table_whole_trans[,start_dis2:=start1-trans_start]
dist_table_whole_trans[,end_dis2:=end1-trans_start]
dist_table_whole_trans[,distance2:=ifelse(start_dis<end_dis,start_dis2,end_dis2)]

dist_table_whole_go_trans_f<-merge(dist_table_whole_trans,prot_overlaps_20kb_dt_GO_longer_f[,.SD,.SDcols=c("attribute","GO_term","GO_category")])

dist_table_whole_go_trans_p<-merge(dist_table_whole_trans,prot_overlaps_20kb_dt_GO_longer_p[,.SD,.SDcols=c("attribute","GO_term","GO_category")])



dist_table_whole_go_trans_f<-dist_table_whole_go_trans_f[GO_term%in%top_go_f]

dist_table_whole_go_trans_p<-dist_table_whole_go_trans_p[GO_term%in%top_go_p]

dist_table_whole_go_trans_f[,GO_term:=factor(GO_term,levels = prot_overlaps_20kb_dt_GO_longer_f_N$GO_term)]
dist_table_whole_go_trans_p[,GO_term:=factor(GO_term,levels = prot_overlaps_20kb_dt_GO_longer_p_N$GO_term)]


ggplot(data=dist_table_whole_go_trans_f,aes(x=GO_term,y=distance,fill=GO_term)) +
  geom_boxplot(alpha=0.9)+
  theme_bw() +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(angle = 45,hjust = 0.9,size = 9)) +
...
ggplot(data=dist_table_whole_go_trans_p,aes(x=GO_term,y=distance,fill=GO_term)) +
  geom_boxplot(alpha=0.9)+
  theme_bw() +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(angle = 45,hjust = 0.9,size = 9)) +
...
blocks_table[,Emu_scaffold:=str_remove_all(Emu_scaffold,"Emu")]
blocks_table[,Emu_scaffold:=str_remove(Emu_scaffold,";")]
blocks_table[,Emu_scaffold:=str_replace(Emu_scaffold,"scaffold(\\d*)","scaffold_\\1")]
blocks_table[,Esu_scaffold:=str_remove_all(Esu_scaffold,"Esu")]
blocks_table[,Esu_scaffold:=str_remove(Esu_scaffold,";")]
...
blocks_table_emu_merged[,Emu_ID:=paste(Emu_scaffold,Emu_span_range_1,sep="_")]
...
setwd("C:/Users/krist/Desktop/1_script/33")
...
plotTracks(list(esu_genome_track,esu_genes_track),shape="arrow",fill=c(rep("steelblue",nrow(prot_sub)),rep("indianred1",nrow(lncrna_sub_dt))),sizes = c(0.5,0.3),from=block_lncrna_merge[block==numb]$start2.x,to=block_lncrna_merge[block==numb]$end2.x,main = paste("Esu",numb,sep=" "),cex.main = 1,cex=2,fontsize.group=30)
...
dist_table_whole_go_trans_f_merge<-merge(dist_table_whole_go_trans_f,synt_cons_lncrna_table,by="gene_id")

dist_table_whole_go_trans_p_merge<-merge(dist_table_whole_go_trans_p,synt_cons_lncrna_table,by="gene_id")
...
dist_table_whole_go_trans_f_merge[,cons_cat:=factor(cons_cat,levels=c("other","conserved","syntenic","syntenic and conserved"))]
dist_table_whole_go_trans_p_merge[,cons_cat:=factor(cons_cat,levels=c("other","conserved","syntenic","syntenic and conserved"))]
...
ggplot(data=dist_table_whole_go_trans_p_merge,aes(x=GO_term,y=distance,fill=cons_cat)) +
  geom_boxplot(alpha=0.9)+
  theme_bw() +
  theme(legend.position = "top") +
  theme(axis.text.x = element_text(angle = 45,hjust = 0.9,size = 9)) +
