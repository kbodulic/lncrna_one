...
less_than_150_orfs_rnaspades<-orf_table_transdecoder_rnaspades_maxlen[max_length<=75]$transcript
less_than_150_orfs_rtrinity<-orf_table_transdecoder_trinity_maxlen[max_length<=75]$transcript
...
rnaspades_no_hmmer<-setdiff(rnaspades_less_than_150_ORF_without_blast,hmmer_rnaspades_besthit[eval_min<=1e-10]$transcript)
trinity_no_hmmer<-setdiff(trinity_less_than_150_ORF_without_blast,hmmer_trinity_besthit[eval_min<=1e-10]$transcript)
...
lengths_plot<-ggplot(data=lengths_table,aes(x=filtering,y=number_of_lncRNA_candidates,fill=Transcriptome)) +
  ...
  scale_x_discrete(labels=rev(c("All transcripts","rRNA filtering","Length filtering (200 bp)","ORF filtering (75 aa)","Protein similarity filtering (DIAMOND)","Protein similarity filtering (HMMER)","FEELnc filtering","Percent identity filtering","Bacterial lncRNA filtering ","One-exon transcript filtering","Protein-coding exons overlap filtering"))) +
   scale_fill_locuszoom() +
  theme(axis.text = element_text(size=4)) +
  theme(axis.title.x = element_text(size=5.5)) +
  theme(strip.text = element_text(size=5))

lengths_plot_legend<-ggplot(data=lengths_table,aes(x=filtering,y=number_of_lncRNA_candidates,fill=Transcriptome)) +
  geom_bar(position="dodge",stat = "identity", width=0.65,col="black",alpha=0.8) +
  theme_bw() +
  facet_grid(row=vars(Transcriptome)) +
...
lengths_plot_legend
ggsave(lengths_plot_legend,filename = "lengths_plot_legend.jpg",width = 7,height = 5)
...
for (i in 1:nrow(overlap_table)) {
  inter_width_vector<-c(inter_width_vector,1+Overlap(c(overlap_table$start1[i],overlap_table$end1[i]),c(overlap_table$start2[i],overlap_table$end2[i])))
}
...
perc_plot<-ggplot(data=overlap_table,aes(x=length_frac))+
  geom_histogram(fill="chocolate1",col="black",alpha=0.85,binwidth = 5) +
  theme_bw() +
...
trinity_paf_filtered_granges_reduced<-makeGRangesFromDataFrame(trinity_paf_filtered_granges_reduced_dt,keep.extra.columns = T)
reduced_lessthan20_shorter_together<-rbind(reduced_lessthan20_shorter_rnaspades,reduced_lessthan20_shorter_trinity)
...
lncrna_numbers_table_plot<-lncrna_numbers_table[category%in%c("RNASpades genes","Trinity genes","Longer RNASpades genes with more than 20% overlap","Longer Trinity genes with more than 20% overlap","Longer RNASpades genes with less than 20% overlap","Longer Trinity genes with less than 20% overlap","Shorter RNASpades genes with less than 20% overlap","Shorter Trinity genes with less than 20% overlap","RNASpades genes without overlaps","Trinity genes without overlap","Final RNASpades genes","Final Trinity genes")]

lncrna_numbers_table_plot[,category:=c("All genes","All genes","Longer genes (>= 20% overlap)","Longer genes (>= 20% overlap)","Longer genes (< 20% overlap)","Longer genes (< 20% overlap)","Shorter genes (< 20% overlap)","Shorter genes (< 20% overlap)","Unique genes","Unique genes","Final genes","Final genes")]
lncrna_numbers_table_plot[,category:=factor(category,levels = c("All genes","Unique genes","Longer genes (>= 20% overlap)","Longer genes (< 20% overlap)","Shorter genes (< 20% overlap)","Final genes"))]
lncrna_numbers_table_plot[,Transcriptome:=rep(c("rnaSPades","Trinity"),6)]
...
plot_consensus<-ggplot(data=lncrna_numbers_table_plot,aes(x=category,y=number,fill=Transcriptome)) +
  ...
  theme_bw() +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(size=4,angle=45,hjust=0.9)) +
...
legend_one<-get_legend(lengths_plot_legend)


figure_one<-ggdraw() +
  draw_plot(lengths_plot, x = 0, y = 0.15, width = .5, height = .62) +
  draw_plot(perc_plot, x = 0.54, y = 0.47, width = .35, height = .3) +
  draw_plot(plot_consensus, x = 0.54, y = 0.11, width = 0.35, height = 0.35) +
  draw_plot(legend_one,x=0.4,y=0.08,width=0.2,height = 0.02) +
  draw_plot_label(label = c("A", "B", "C"), size = 7,
                  x = c(0.01, 0.55, 0.55), y = c(0.8, 0.8, 0.48))

figure_one

ggsave(figure_one,filename = "figure_one.tiff",height = 12,width = 16.99,units = "cm",dpi=320)
#mapping analysis
...
prot_transcript_for_isoforms[,gene:=str_extract(attribute,"(?<=gene_id ).*")]
prot_transcript_for_isoforms[,gene:=str_remove_all(gene,'"')]
...
isoforms_plot_proportions<-ggplot(data=isoforms_table_together_num,aes(x=number_of_isoforms,y=proportion,fill=category)) +
  geom_bar(col="black",position="dodge",stat="identity",width=0.5,alpha=0.8) +
  theme_bw() +
...
rnaspades_filtered_exons<-rnaspades_filtered_exons[transcript%in%final_lncrna_list_rnaspades]
rnaspades_filtered_exons_copy<-copy(rnaspades_filtered_exons)
trinity_filtered_exons<-trinity_filtered_exons[transcript%in%final_lncrna_list_trinity]
...
isoform_overlap_table_rnaspades_merged[,is_max_exons_iso:=ifelse(.SD==max(.SD),T,F),.SDcols="number_of_exons",by=c("seqnames2","start2","end2","gene_id")]
isoform_overlap_table_rnaspades_merged[is_max_exons_iso==T,max_len:=ifelse(.SD==max(.SD),T,F),.SDcols="transcript_length",by=c("seqnames2","start2","end2","gene_id")]
isoform_overlap_table_trinity_merged[,is_max_exons_iso:=ifelse(.SD==max(.SD),T,F),.SDcols="number_of_exons",by=c("seqnames2","start2","end2","gene_id")]
isoform_overlap_table_trinity_merged[is_max_exons_iso==T,max_len:=ifelse(.SD==max(.SD),T,F),.SDcols="transcript_length",by=c("seqnames2","start2","end2","gene_id")]
...
together_longest_iso<-c(rnaspades_longest_iso,trinity_longest_iso)
...
protein_transcripts<-protein_coding_annotation[max_length==T]
...
exons_plot_proportion_legend<-ggplot(data=exon_number_table_together_num,aes(x=number_of_exons,y=proportion,fill=category)) +
  geom_bar(col="black",position="dodge",stat="identity",width=0.5,alpha=0.8) +
  theme_bw() +
  xlab("Number of exons per gene") +
  ylab("Percentage of genes") +
...
exons_plot_proportion_legend
ggsave(exons_plot_proportion_legend,file="exons_plot_proportion_legend.jpg",width=6, height = 4.2) 
...
element_length_boxplot<-ggplot(data=element_width_table,aes(x=category,y=width,fill=category)) +
  ...
  ylab("Length / bp")  +
  stat_compare_means(comparisons = features_comparison,label = "p.signif",size=2,vjust=0.2) +
  theme(axis.text.y = element_text(size=4)) +
...
legend_two<-get_legend(exons_plot_proportion_legend)
...
figure_two<-ggdraw() +
  ...
  draw_plot(element_length_boxplot, x = 0.15, y = 0.02, width = 0.65, height = 0.45) +
  draw_plot(legend_two,x=0.45,y=0.89,width = 0.1,height = 0.1) +
  draw_plot_label(label = c("A", "B", "C"), size = 7,
                  x = c(0.14, 0.52, 0.12), y = c(0.9, 0.9, 0.5))

figure_two

ggsave(figure_two,filename = "figure_two.tiff",height = 12,width = 16.99,units="cm",dpi=320)
...
overlapping_lncrna<-c(overlapping_lncrna,unique(overlaps_candidates_exons_prot_introns_table$transcript))
...
number_of_lncrna_classes_plot<-ggplot(data=number_of_lncrna_classes,aes(x=class,y=number_of_lncRNA,fill=class)) +
  ...
  theme_bw() +
  theme(legend.position = "none") + 
      scale_fill_brewer(palette = "Set2", name = "Class",labels=c("Intronic lncRNA","Intergenic lncRNA","Overlapping lncRNA"))+  
  theme(axis.title.x=element_blank()) +
  ylab("Number of genes") +
    theme(axis.text.x = element_blank()) +
  theme(axis.text.y = element_text(size=4)) +
...
class_genes_rnaspades<-unique(overlaps_table_rnaspades_with_class[,.SD,.SDcols=c("seqnames2","start2","end2","width2","class")])
class_genes_trinity<-unique(overlaps_table_trinity_with_class[,.SD,.SDcols=c("seqnames2","start2","end2","width2","class")])
class_genes_table<-rbind(class_genes_rnaspades,class_genes_trinity)
class_genes_table<-class_genes_table[,.SD,.SDcols=c("width2","class")]
...
class_lengths_plot<-ggplot(data=class_together_table,aes(x=class,y=element_length,fill=class)) +
  ...
  scale_fill_brewer(palette = "Set2", name = "Class",labels=c("Intronic lncRNA","Intergenic lncRNA","Overlapping lncRNA"))+
  ylab("Length / bp")  +
  stat_compare_means(comparisons = class_comparison,label = "p.signif",size=2,vjust=0.2) +
  theme(strip.text = element_text(size=5)) +
    theme(axis.title=element_text(size=5))  +
  theme(axis.text.y=element_text(size=4))
...
class_genes_table_sum_width_plot<-ggplot(data=class_genes_table_sum_width,aes(x=class,y=bases_sum_kb,fill=class)) +
  ...
  class_genes_table_sum_width_plot_legend<-ggplot(data=class_genes_table_sum_width,aes(x=class,y=bases_sum_kb,fill=class)) +
          geom_bar(color="black",width=0.5,stat="identity") +
  theme_bw() +
  theme(legend.position = "bottom") + 
    scale_fill_brewer(palette = "Set2",labels=c("Intronic lncRNA","Intergenic lncRNA","Overlapping lncRNA"),name="Class") +
  theme(axis.title.x=element_blank()) +
  ylab("Total length / kb") +
  theme(axis.text.y = element_text(size=4)) +
  theme(axis.title=element_text(size=5)) +
  ...
  class_genes_table_sum_width_plot_legend
  
  ggsave(class_genes_table_sum_width_plot_legend,filename = "class_genes_table_sum_width_plot_legend.jpg",height = 4,width = 6)
...
legend_three<-get_legend(class_genes_table_sum_width_plot_legend)

figure_three<-ggdraw() +
  draw_plot(number_of_lncrna_classes_plot, x = 0.1615, y = 0.47, width = .27, height = .36) +
  draw_plot(class_genes_table_sum_width_plot, x = 0.445, y = 0.47, width = .27, height = .36) +
  draw_plot(class_lengths_plot, x = 0.13, y = 0.03, width = 0.6, height = 0.42) +
  draw_plot(legend_three,x=0.415,y=0.83,width = 0.1,height = 0.1) +
  draw_plot_label(label = c("A", "B", "C"), size = 7,
                  x = c(0.15, 0.45, 0.15), y = c(0.85, 0.85, 0.48))

figure_three

ggsave(figure_three,filename = "figure_three.tiff",height = 12,width = 16.99,units="cm",dpi=320)
...
setnames(class_gene_together,c("seqnames1","start1","end1","width1","class"))
lncrnas_final_reduced_together_dt_merged<-merge(lncrnas_final_reduced_together_dt,class_gene_together,by=c("seqnames1","start1","end1","width1"))
...
intronic_prots<-unique(overlaps_lncRNA_prot_within_table[query_name%in%intronic_lncrna]$attribute)

#overlapping

overlapping_prots<-unique(overlaps_prot_lncrna_within_table[query_name%in%overlapping_lncrna]$attribute)
...
lncrna_filtered_introns_clean_dt_gr_split_red_un<-unlist(lncrna_filtered_introns_clean_dt_gr_split_red,use.names = T)
lncrna_filtered_introns_clean_dt_gr_split_red_un$gene_id<-as.numeric(names(lncrna_filtered_introns_clean_dt_gr_split_red_un))
...
lncrna_filtered_exons_clean_dt_gr_split_red_un<-unlist(lncrna_filtered_exons_clean_dt_gr_split_red,use.names = T)
lncrna_filtered_exons_clean_dt_gr_split_red_un$gene_id<-as.numeric(names(lncrna_filtered_exons_clean_dt_gr_split_red_un))
...
dt_lncrna_right_dt[,start.x:=end.y+1]
...
dt_prot_right_dt[,start.x:=end.y+1]
...
perc_within_trans<-ggplot(data=overlaps_repeat_together_num_table,aes(x=class,y=percentage_genes_within,fill=class)) +
  ...
  theme(axis.text.y = element_text(size=4)) +
  scale_x_discrete(labels=c("Intronic lncRNAs","Intergenic lncRNAs","Overlapping lncRNAs","Protein-coding genes")) +
    theme(axis.title.x = element_blank()) +
  ylab("Percentage of genes") +
  theme(axis.title.y = element_text(size=6)) +
  scale_fill_manual(values=c(brewer.pal(n=3,name = "Set2"),"steelblue3"))
...
for (i in 1:nrow(overlaps_repeats_lncrna_bases_table)) {
  inter_width_vector<-c(inter_width_vector,1+Overlap(c(overlaps_repeats_lncrna_bases_table$start1[i],overlaps_repeats_lncrna_bases_table$end1[i]),c(overlaps_repeats_lncrna_bases_table$start2[i],overlaps_repeats_lncrna_bases_table$end2[i])))
}
...
for (i in 1:nrow(overlaps_repeats_exons_table_20)) {
  inter_width_vector<-c(inter_width_vector,1+Overlap(c(overlaps_repeats_exons_table_20$start1[i],overlaps_repeats_exons_table_20$end1[i]),c(overlaps_repeats_exons_table_20$start2[i],overlaps_repeats_exons_table_20$end2[i])))
}
...
for (i in 1:nrow(overlaps_repeats_protein_exons_table_20_prot)) {
  inter_width_vector<-c(inter_width_vector,1+Overlap(c(overlaps_repeats_protein_exons_table_20_prot$start1[i],overlaps_repeats_protein_exons_table_20_prot$end1[i]),c(overlaps_repeats_protein_exons_table_20_prot$start2[i],overlaps_repeats_protein_exons_table_20_prot$end2[i])))
}
...
overlaps_repeat_together_num_table_normalized_sum_bases_20[class=="intergenic_lncRNA",class:="Intergenic lncRNAs"]
overlaps_repeat_together_num_table_normalized_sum_bases_20[class=="intronic_lncRNA",class:="Intronic lncRNAs"]
overlaps_repeat_together_num_table_normalized_sum_bases_20[class=="overlapping_lncRNA",class:="Overlapping lncRNAs"]
...
repeats_plot_normalized_sum_bases_20_rel<-ggplot(data=overlaps_repeat_together_num_table_normalized_sum_bases_20,aes(x=repeat_class,y=number_of_bases_rel,fill=category)) +
  geom_bar(position=position_dodge(preserve = "single"),stat = "identity", width=0.7,alpha=0.9) +
  theme_bw() +
  facet_grid(row=vars(class)) +
  theme(legend.position = "none") + 
  #theme(axis.text.x = element_text(angle = 50, hjust = 1,size=5)) +
  xlab("Transposon class") +
  ylab("(Observed - Expected) / Observed") +
  labs(fill = "lncRNA element",size=5) +
  theme(axis.title=element_text(size=6)) +
  theme(axis.text=element_text(size=4))+
  theme(legend.title = element_text(size=10,face="bold")) +
  theme(legend.text = element_text(size=10)) +
 scale_fill_manual(values = wes_palette("Rushmore1", n = 3),name="Element",labels=c("Proximity","Exon","Intron")) +
...
repeats_plot_normalized_sum_bases_20_2<-ggplot(data=overlaps_repeat_together_num_table_normalized_sum_bases_20_2,aes(x=class,y=number_of_bases,fill=category)) +
  geom_bar(position="stack",stat = "identity",col="black",alpha=0.9,width=0.8) +
  theme_bw() +
  theme(legend.position = "none") + 
  #theme(axis.text.x = element_text(angle = 50, hjust = 1,size=5)) +
  xlab("Gene class") +
...
repeats_plot_normalized_sum_bases_20_2_legend<-ggplot(data=overlaps_repeat_together_num_table_normalized_sum_bases_20_2,aes(x=class,y=number_of_bases,fill=category)) +
  geom_bar(position="stack",stat = "identity",col="black",alpha=0.9,width=0.8) +
  theme_bw() +
  theme(legend.position = "bottom") + 
  #theme(axis.text.x = element_text(angle = 50, hjust = 1,size=5)) +
...
repeats_plot_normalized_sum_bases_20_2_legend

ggsave(repeats_plot_normalized_sum_bases_20_2_legend,file="repeats_plot_normalized_sum_bases_20_2_legend.jpg",width=7.6,height = 3.5)


legend_four<-get_legend(repeats_plot_normalized_sum_bases_20_2_legend)
...
figure_four<-ggdraw() +
  ...
  draw_plot(repeats_plot_normalized_sum_bases_20_2, x = 0.05, y = 0.08, width = 0.35, height = 0.4) +
  draw_plot(repeats_plot_normalized_sum_bases_20_rel, x = 0.47, y = 0.15, width = 0.52, height = 0.65) +
  draw_plot(legend_four,x=0.46,y=0.02,width = 0.1,height = 0.1) +
  draw_plot_label(label = c("A", "B", "C","D"), size = 7,
                  x = c(0.035, 0.27, 0.05,0.5), y = c(0.88, 0.88, 0.495,0.83))


figure_four

ggsave(figure_four,filename = "figure_four.tiff",height = 13,width = 16.99,units = "cm",dpi = 320)
...
blast_results_sponge_transcriptomes_eval_filt2<-copy(blast_results_sponge_transcriptomes_eval_filt)
blast_results_sponge_transcriptomes_eval_filt2[species=="Ephydatia_mullerig_genome",species:="Ephydatia_mulleri_transcriptome"]
blast_results_sponge_transcriptomes_eval_filt2[,number_of_species_per_gene:=uniqueN(.SD),.SDcols="species",by=qseqid]



numb_gene_in_genomes_table<-blast_results_sponge_transcriptomes_eval_filt2[,uniqueN(qseqid),by=number_of_species_per_gene]
setnames(numb_gene_in_genomes_table,c("number_of_genomes","number_of_lncrna"))
total_lncrna_num<-lncrna_numbers_table[category=="Final genes"]$number
...
species_ordered3[species_ordered3=="Ephydatia_mullerig"]<-"Ephydatia_muelleri" 
species_ordered3[species_ordered3=="Ephydatia_mulleri"]<-"Ephydatia_muelleri" 
species_ordered3<-str_replace(species_ordered3,"_"," ")
blast_gr_q_split_reduce_unlist_dt_lengths_one_per_species[,qseqid:=factor(qseqid,levels=unique(blast_gr_q_split_reduce_unlist_dt_lengths_one_per_species$qseqid))]
blast_heatmap_plot<-ggplot(data=blast_gr_q_split_reduce_unlist_dt_lengths_one_per_species,aes(y=species,x=qseqid,fill=sumqperclen)) +
  ...
  theme(axis.text.y = element_text(size=4,colour = rev(label_cols))) 
...
aln<-readJPEG("aln.jpg")
rna<-readJPEG("rna.jpg")
aln <- rasterGrob(aln, interpolate=TRUE,width = 1.4,height = 0.5)
rna <- rasterGrob(rna, interpolate=TRUE,width=0.5,height = 1)


figure_five<-ggdraw() +
  draw_plot(numb_gene_in_genomes_plot, x = 0, y = 0.42, width = .35, height = .38) +
  draw_plot(blast_heatmap_plot, x = .36, y = 0.35, width = .7, height = .63) +
  draw_plot(aln,x=0.15,y=0.1,width = 0.4,height = 0.2)+
  draw_plot(rna,x=0.58,y=0.05,width = 0.4,height = 0.3) +
  draw_plot_label(label = c("A", "B","C"), size = 7,
                  x = c(0.02, 0.46,0.06), y = c(0.81, 0.94,0.34))


figure_five

ggsave(figure_five,filename = "figure_five.tiff",height = 14,width = 16.99,units  ="cm",dpi=320)
...
fpkm_lncrna_class_plot<-ggplot(data=read_counts_table_lncrna_class,aes(x=class,y=FPKM,fill=class)) +
  geom_boxplot(width=0.45,outlier.size=0.5) +
  theme_bw() +
  scale_y_log10() +
  scale_fill_brewer(palette = "Set2") +
  theme(legend.position = "none") +
  ...
  theme(axis.text = element_text(size=4)) +
  theme(axis.title=element_text(size=5)) +
  scale_x_discrete(labels=c("Intronic lncRNAs","Intergenic lncRNAs","Overlapping lncRNAs")) +
  stat_compare_means(comparisons = fpkm_class_comp,label="p.signif",size=2.2)
...
fpkm_lncrna_repeats_plot<-ggplot(data=lncrna_repeat_overlap_category_class_merged_copy_merged_with_no_rep,aes(x=repeat_class,y=FPKM,fill=element)) +
  ...
  theme(axis.text = element_text(size=4)) +
  theme(axis.title=element_text(size=5)) +
  scale_fill_manual(values = wes_palette("Rushmore1", n = 3),name="Element",labels=c("Proximity","Exon","Intron"),na.value="grey") +
  xlab("TE class") +
  #theme(axis.text.x=element_blank()) +
  theme(legend.position = "none") +
...
options(scipen=1000)
fpkm_prot_comp<-list(c("no_lncRNA","intronic"),c("no_lncRNA","intergenic_1kb"),c("no_lncRNA","overlapping"))


pal <- rev(wes_palette("Royal1", 4, type = "discrete"))
...
fpkm_prot_plot<-ggplot(data=read_counts_table_longer_prot_merged,aes(x=lncrna_class,y=FPKM,fill=lncrna_class)) +
  ...
  theme(axis.title.x=element_blank()) +
  theme(axis.text.x =element_blank()) +
  scale_fill_manual(values=pal,name = "Category",labels=c("Proteins containing\n an intronic lncRNA","Protein with an intergenic\nlncRNA closer than 1 kb","Proteins in\nan overlapping lncRNA","Other\nproteins")) +
  theme(legend.text = element_text(size=5)) +
  stat_compare_means(comparisons = fpkm_prot_comp,label="p.signif",symnum.args = list(cutpoints = c(0, 1E-15, 0.001, 0.01, 0.05, 1), symbols = c("****", "***", "**", "*", "ns")),size=2.2) +
  theme(axis.title.y = element_text(size=5)) +
  theme(axis.text = element_text(size=4)) 


fpkm_prot_plot_flip<-ggplot(data=read_counts_table_longer_prot_merged,aes(x=lncrna_class,y=FPKM,fill=lncrna_class)) +
  geom_boxplot(outlier.size = 0.5,width=0.38) +
  theme_bw() +
  scale_y_log10() +
  theme(legend.title = element_text(size=6,face="bold")) +
  scale_fill_manual(values=pal,name = "Category",labels=c("Proteins containing\n an intronic lncRNA","Protein with an intergenic\nlncRNA closer than 1 kb","Proteins in\nan overlapping lncRNA","Other\nproteins")) +
...
fpkm_prot_plot_flip
ggsave(fpkm_prot_plot_flip,file="fpkm_prot_plot_flip.png",width = 7,height = 4)



fpkm_prot_plot_flip_legend<-ggplot(data=read_counts_table_longer_prot_merged,aes(x=lncrna_class,y=FPKM,fill=lncrna_class)) +
  geom_boxplot(outlier.size = 0.5,width=0.38) +
  theme_bw() +
  scale_y_log10() +
  scale_fill_manual(values=pal,name = "Category",labels=c("PCGs containing\nan intronic lncRNA","PCGs with an intergenic\nlncRNA closer than 1 kb","PCGs in an\noverlapping lncRNA","Other\nPCGs"))+
  theme(legend.text = element_text(size=5)) +
  stat_compare_means(comparisons = fpkm_prot_comp,label="p.signif",symnum.args = list(cutpoints = c(0, 1E-15, 0.001, 0.01, 0.05, 1), symbols = c("****", "***", "**", "*", "ns")),size=2.2) +
  coord_flip() +
  theme(legend.position = "bottom") +
...
fpkm_prot_plot_flip_legend
ggsave(fpkm_prot_plot_flip_legend,file="fpkm_prot_plot_flip_legend.jpg",width = 7,height = 4)
...
figure_six<-ggdraw() +
  draw_plot(fpkm_lncrna_class_plot, x = 0, y = 0.5, width = .42, height = .4) +
  draw_plot(fpkm_lncrna_repeats_plot, x = 0, y = 0.05, width = .42, height = .42) +
  draw_plot(fpkm_prot_plot, x = .45, y = 0.25, width = .5, height = .5) +
  draw_plot_label(label = c("A", "B","C"), size = 7,
                  x = c(0.02, 0.02,0.49), y = c(0.915, 0.48,0.78))

figure_six

ggsave(figure_six,filename = "figure_six.tiff",height = 12,width = 16.99,units = "cm",dpi=320)
...
unique(blast_gr_q_split_reduce_unlist_dt_lengths_ephy_uniq$qseqid)

blast_gr_q_split_reduce_unlist_dt_lengths_ephy_uniq2<-blast_gr_q_split_reduce_unlist_dt_lengths_ephy_uniq[lncrna_emu%in%lncrnas_final_reduced_together_dt_emu$emu_lncrna]

unique(blast_gr_q_split_reduce_unlist_dt_lengths_ephy_uniq2$qseqid)
```
...
go_all_esu_nonempty<-go_all_esu_nonempty[attribute%in%protein_coding_annotation$attribute]
go_all_esu_nonempty[,go_names:=str_replace_all(go_names,"guanyl-nucleotide exchange factor activity","GTP exchange activity")]
go_all_esu_nonempty[,go_names:=str_replace_all(go_names,"obsolete oxidation-reduction process","oxidation-reduction process")]
go_all_esu_nonempty[,go_names:=str_replace_all(go_names,"regulation of transcription, DNA-templated","regulation of transcription")]
...
prot_overlaps_20kb_dt_GO_longer_f[GO_term=="guanyl-nucleotide exchange factor activity",GO_term:="GTP exchange activity"]
...
p_values_f<-c()
for(i in enr_f_set) {
  a<-go_all_esu_nonempty[grepl(i,go_names),.N]
  b<-nrow(go_all_esu_nonempty)-a
...
synteny_table_merged_prot[,Emu_start:=Emu_start-200]
synteny_table_merged_prot[,Emu_end:=Emu_end+200]
synteny_table_merged_prot[,Esu_start:=Esu_start-200]
synteny_table_merged_prot[,Esu_end:=Esu_end+200]
...
cons_lncrna<-unique(isoform_map_table_together[transcript_name%in%blast_gr_q_split_reduce_unlist_dt_lengths_one_per_species[is.na(sumqperclen)==F]$qseqid]$gene_id)
...
num_f_cons<-ggplot(data=prot_overlaps_20kb_dt_GO_longer_f_merged_N,aes(x=GO_term,N,y=N,fill=cons_cat)) +
  ...
  theme(axis.title=element_text(size=4.1)) +
  theme(axis.title.y = element_blank())
...
ratio_p_cons_legend<-ggplot(data=prot_overlaps_20kb_dt_GO_longer_p_merged_N,aes(x=GO_term,y=N,fill=cons_cat)) +
  geom_bar(stat="identity",color="black",alpha=0.7,position="fill",width=0.7) +
  theme_bw() +
  theme(legend.position = "bottom") +
  theme(axis.text.x = element_text(angle = 45,hjust = 0.9,size = 7)) +
  ...
  theme(legend.text = element_text(size=5)) +
   theme(legend.key.size = unit(0.4, 'cm')) +
    scale_fill_brewer(palette="Spectral",name="lncRNA category",labels=c("Conserved","Syntenic","Syntenic and conserved","Other")) 

ratio_p_cons_legend

ggsave(ratio_p_cons_legend,filename = "ratio_p_cons_legend.jpg",height = 6,width = 9)
...
prot_overlaps_20kb_dt_GO_longer_p_merged_class_N[class=="intronic_lncRNA",class:="Intronic lncRNAs"]
prot_overlaps_20kb_dt_GO_longer_p_merged_class_N[class=="intergenic_lncRNA",class:="Intergenic lncRNAs"]
prot_overlaps_20kb_dt_GO_longer_p_merged_class_N[class=="overlapping_lncRNA",class:="Overlapping lncRNAs"]
num_p_cons_class<-ggplot(data=prot_overlaps_20kb_dt_GO_longer_p_merged_class_N,aes(x=GO_term,y=N,fill=cons_cat)) +
  ...
  ylab("Number of protein-coding genes") +
    coord_flip() +
  theme(axis.text= element_text(size=4)) +
  theme(axis.title=element_text(size=3.5)) +
  theme(axis.title.y = element_blank()) +
  theme(strip.text = element_text(size=3.9)) 
...
prot_overlaps_20kb_dt_GO_longer_p_merged2_exp[,GO_term:=factor(GO_term,levels=rev(top_go_p))]
...
legend_seven<-get_legend(ratio_p_cons_legend)
legend_sevend<-get_legend(fpkm_prot_plot_flip_legend)

figure_seven<-ggdraw() +
  draw_plot(num_f_cons, x = 0.01, y = 0.47, width = .24, height = .45) +
  draw_plot(num_p_cons, x = .25, y = 0.47, width = .26, height = .45) +
  draw_plot(fpkm_prot_plot_flip, x = 0.1, y = 0.05, width = 0.35, height = 0.4) +
  draw_plot(num_p_cons_class, x = 0.52, y = 0.47, width = 0.47, height = 0.45) +
  draw_plot(go_expression_plot,x=0.5,y=0.05,width=0.42,height = 0.4) +
  draw_plot(legend_seven,x=0.45,y=0.905,width = 0.1,height = 0.1) +
  draw_plot(legend_sevend,x=0.25,y=0.0,width = 0.1,height = 0.1) +
  draw_plot_label(label = c("A", "B", "C","D","E"), size =7,
                  x = c(0.01, 0.25, 0.51,0.07,0.47), y = c(0.92, 0.92, 0.92,0.47,0.47))

figure_seven

ggsave(figure_seven,filename = "figure_seven.tiff",height = 12,width = 16.99,dpi=320,units = "cm")
```
