...
gc_density_plot<-ggplot(data=gc_table,aes(x=transcript_gc,y=..scaled..,fill=category)) +
  ...
  scale_fill_manual(values=c("indianred2","steelblue"), name = "Category", labels=c("lncRNA","mRNA")) +
  xlab("GC content") +
...
gc_exons_lncrna<-100*(alphabetFrequency(lncrna_exon_sequences,as.prob = T)[,"C"]+alphabetFrequency(lncrna_exon_sequences,as.prob = T)[,"G"])
gc_introns_lncrna<-100*(alphabetFrequency(lncrna_intron_sequences,as.prob = T)[,"C"]+alphabetFrequency(lncrna_intron_sequences,as.prob = T)[,"G"])
gc_exons_prot<-100*(alphabetFrequency(protein_exons_sequences,as.prob = T)[,"C"]+alphabetFrequency(protein_exons_sequences,as.prob = T)[,"G"])
gc_introns_prot<-100*(alphabetFrequency(protein_intron_sequences,as.prob = T)[,"C"]+alphabetFrequency(protein_intron_sequences,as.prob = T)[,"G"])

gc_table_box<-data.table(transcript_gc=c(gc_exons_lncrna,gc_introns_lncrna,gc_exons_prot,gc_introns_prot),category=c(rep("lncRNA_exons",length(gc_exons_lncrna)),rep("lncRNA_introns",length(gc_introns_lncrna)),rep("protein_exons",length(prot_gc)),rep("protein_introns")))

gc_table_box[,cate:=ifelse(grepl("lncRNA",category),"lncRNA","protein")]
gc_table_box[,element:=ifelse(grepl("exons",category),"Exons","Introns")]


gc_box<-ggplot(gc_table_box,aes(x=cate,y=transcript_gc,fill=cate)) +
  geom_boxplot(width=0.75,alpha=0.8) +
  theme_bw() +
  facet_wrap(.~element) +
  ...
  theme(legend.position = "none") +
  ylab("GC content") +
  geom_hline(yintercept = genome_gc,linetype="dotted",size=1)

gc_box

ggsave(gc_box,filename = "gc_box.png",height = 4,width = 6)
